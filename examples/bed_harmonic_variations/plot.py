#!/usr/bin/env python3
"""
Visualization for the bed harmonic variations example.

Reads output files from the sweep and produces the analysis plot.
Can be run standalone: uv run examples/bed_harmonic_variations/plot.py [output_dir]
"""
...
def plot(output_dir):
    ...
    print("Creating bed harmonic analysis...")
    ...
    subplot_mosaic = [
        ["bed_profiles",    "bed_profiles",    "length_response",  "length_response",  "f_eq",            "f_eq",            "smb",         "smb"        ],
    ...
    fig.suptitle("Bed Shape Effects on Glacier Dynamics: Harmonic Profiles (Hourglass+Convex vs Oval+Concave)", fontsize=16)

    # 1. Bed elevation profiles
    for profile_type in ordered_profiles:
        if profile_type in ds.coords["run_id"].values:
            zb = ds["zb"].sel(run_id=profile_type)
            axes["bed_profiles"].plot(
                ds.coords["x"] / 1000,
                zb,
    ...
    axes["bed_profiles"].set_xlabel("Distance from head (km)")
    axes["bed_profiles"].set_ylabel("Bed elevation (m)")
    axes["bed_profiles"].set_title("Bed Elevation Profiles")
    axes["bed_profiles"].legend()
    axes["bed_profiles"].grid(True, alpha=0.3)
    ...
    if "edge" in ds.data_vars:
        ...
        axes["length_response"].set_title("Length Response to +0.5°C Warming")
    ...
    if "h" in ds.data_vars and "w" in ds.data_vars:
        ...
        axes["volume_response"].set_title("Absolute Volume Response to +0.5°C Warming")
    ...
    if tau_theory or tau_fitted:
        ...
        ax_tau.set_title("Response Timescale by Bed Shape")
        ax_tau.legend()
    ...
    plot_path = output_dir / "bed_harmonic_analysis.png"
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Analysis plot saved to: {plot_path}")

    if spinup_T0_values and initial_lengths and final_lengths:
        print("\nBed Harmonic Analysis Summary:")
        print("  All profiles: constant width=1250m; bed perturbation A * sin^2(pi*x/L)")
        ...
        print("\n  Final lengths (after +0.5°C):")
        for i, name in enumerate(profile_names):
            print(f"    {name}: {final_lengths[i]:.1f}km")
        print("\n  Length change from +0.5°C warming:")
        for i, name in enumerate(profile_names):
