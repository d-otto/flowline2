...
from flowline.visualization import plot_thickness_profiles
...
def main():
    ...
    for i, gamma_val in enumerate(gamma_values):
        ...
        spinup_forcing = TemperaturePrecipitationForcing(
            T0=8.0,  # Initial guess temperature
            P0=2.0,
            mu=0.65,  # melt factor
            gamma=gamma_val,  # Different gamma for each run
            ts=spinup_config.ts,
            tf=spinup_config.tf,
        )
        ...
        target_matching = {
            ...
            "bounds": (5.0, 11.0),  # renamed + wrapped in list
            "max_simulation_time": spinup_config.tf,
            "optimization_options": {
                "maxiter": 25,  # nested under optimization_options
            },
        }
        ...
        logger.debug(
            f"Created spinup {run_id} with initial T0={spinup_obj.forcing.T0:.3f}, gamma={spinup_obj.forcing.gamma:.6f}"
        )
    ...
    print(
        f"Expected: Both runs should achieve ~{target_length}m length with different T0 values"
    )
    ...
    if results_file.exists():
        ...
        for i, run_id in enumerate(ds.run_id.values):
            ...
            print(
                f"Run {run_id} (mu={gamma_val}): Initial length = {initial_length:.1f}m"
            )

        # Create thickness profiles plot
        print("\\nCreating thickness profiles plot...")
        thickness_plot_path = output_dir / "thickness_profiles.png"
        plot_thickness_profiles(ds, save_path=thickness_plot_path)
        print(f"Thickness profiles saved to: {thickness_plot_path}")
