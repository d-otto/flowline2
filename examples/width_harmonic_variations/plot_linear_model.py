#!/usr/bin/env python3
"""
Linear model comparison plot for width harmonic variations.
...
def plot(output_dir):
    ...
    fig.suptitle("Width Harmonic Variations: Linear Model Comparison", fontsize=16)
    ...
    if tau:
        ...
        profiles_with_tau = [p for p in ordered_profiles if p in tau]
        profile_labels = [p.replace("_", " ").title() for p in profiles_with_tau]
        x_pos = np.arange(len(profile_labels))

        for i, p in enumerate(profiles_with_tau):
    ...
    if "edge" in ds.data_vars:
        ...
        axes["length_response"].set_title("Length Response to Warming")
    ...
    plot_path = output_dir / "width_harmonic_linear_model.png"
