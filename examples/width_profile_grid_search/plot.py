#!/usr/bin/env python3
"""
Plots for the width_profile_grid_search example.

Produces:
  1. scatter_T0_vs_length.png  — steady-state length vs temperature for each profile
  2. branch_{profile}.png      — per-profile diagnostics: length fractional equilibration,
...
PROFILES = ["top_heavy", "neutral", "bottom_heavy"]
PROFILE_COLORS = {"top_heavy": "red", "neutral": "green", "bottom_heavy": "blue"}
...
def _load_spinup(spinup_dir, profile):
    ...
    # is {profile}_dT_m1.00 since DT_VALUES starts at -1.0.
    candidates = sorted(spinup_dir.glob(f"spinup_spinup_{profile}_dT_*.nc"))
    if not candidates:
        return None
...
def compute_linear_model_quantities(ds, spinup_ds, profile, run_id, delx):
    """
...
def _read_spinup_T0(spinup_dir, profile):
    """Extract optimized T0 from spinup profile attributes (stored as forcing_T0)."""
    sp = _load_spinup(spinup_dir, profile)
    if sp is None:
        return None
...
def plot_scatter_T0_vs_length(ds, spinup_dir, output_dir):
    """Scatter: equilibrium T0 (spinup T0 + dT) vs steady-state length, one line per profile.
    Second subplot shifts each line so the spinup T0 maps to 0 and the spinup length to 8 km,
    putting all profiles on a common reference frame."""
    fig, axes = plt.subplot_mosaic([["abs", "rel"]], figsize=(16, 6))

    have_absolute_T0 = False
    profile_data = {}
    for profile in PROFILES:
        T0_spinup = _read_spinup_T0(spinup_dir, profile)
        ...
        profile_data[profile] = {"T0_vals": T0_vals, "length_vals": length_vals, "T0_spinup": T0_spinup}
...
def plot_branch(ds, spinup_dir, profile, delx, output_dir):
    """Per-profile figure: length fractional equilibration, length change, total flux, volume, and linear model scatter."""
    ...
    fig.suptitle(f"Width Profile: {profile.replace('_', ' ').title()}", fontsize=14)
    ...
    for dT in DT_VALUES:
        ...
        if sp is not None and "h" in ds.data_vars:
            lm = compute_linear_model_quantities(ds, sp, profile, run_id, delx)
            lm_results[dT] = lm
    ...
    fig.tight_layout()
    out_path = output_dir / f"branch_{profile}.png"
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    print(f"Saved: {out_path}")

    # --- Print diagnostics and return rows for CSV ---
    return _print_diagnostics(profile, lm_results, weighted_betas, weighted_taus, frac_L_series, frac_flux_series, frac_vol_series, frac_flux_per_vol_series)


def _print_diagnostics(profile, lm_results, weighted_betas, weighted_taus, frac_L_series, frac_flux_series, frac_vol_series, frac_flux_per_vol_series):
    """Print a table of linear model and curve-fit diagnostics for one profile.
    Returns a list of dicts (one per dT row) for CSV export."""
    print(f"\n{'='*130}")
    print(f"Diagnostics: {profile.replace('_', ' ').title()}")
    print(f"{'='*130}")
...
def plot_beta_tau_table(all_rows, output_dir):
    ...
    Rows = profile. Colors distinguish variants within each panel.
    ...
    # --- beta plot: rows = profile, colors = variant ---
    n = len(PROFILES)
    fig, axes = plt.subplot_mosaic([[p] for p in PROFILES], figsize=(9, 4 * n))
    fig.suptitle(r"$\beta$ variants by profile", fontsize=13)

    for profile in PROFILES:
        ax = axes[profile]
        rows_p = sorted(by_profile[profile], key=lambda r: r["dT"])
        dTs = np.array([r["dT"] for r in rows_p])
        dL_real = np.array([r.get("dL_real", np.nan) for r in rows_p], dtype=float)
        ax.plot(dTs, dL_real, "k-", linewidth=2, label="dL real (full model)")
        for key, label, color in BETA_VARIANTS:
            vals = np.array([r.get(key, np.nan) for r in rows_p], dtype=float)
    ...
    axes[PROFILES[-1]].set_xlabel("dT (°C)")
    fig.tight_layout()
    ...
    # --- tau plot: rows = method, colors = profile ---
    ...
    for key, label, _ in TAU_VARIANTS:
        ...
        for profile in PROFILES:
            ...
            if np.all(np.isnan(vals)):
                continue
            ax.plot(dTs, vals, "-o", color=PROFILE_COLORS[profile], linewidth=1.5, markersize=5,
                    label=profile.replace("_", " ").title())
        ax.axvline(0, color="k", linewidth=0.5, linestyle="--", alpha=0.4)
    ...
    fig.legend(handles, labels, loc="center right", bbox_to_anchor=(1.0, 0.5),
               fontsize=9, framealpha=0.9, title="Profile", title_fontsize=9)
...
def plot(output_dir):
    ...
    all_rows = []
    for profile in PROFILES:
        profile_runs = [r for r in run_ids if r.startswith(profile)]
        if not profile_runs:
            print(f"No runs found for profile '{profile}', skipping.")
            continue
        print(f"Plotting branch figure for '{profile}' ({len(profile_runs)} runs)...")
        rows = plot_branch(ds, spinup_dir, profile, delx, output_dir)
        if rows:
