...
class FlowlineSweep:
    ...
    def __init__(
        ...
        if self.spinup_config and self.spinup_objects:
            raise ValueError(
                "Cannot specify both spinup_config and spinup_objects. "
    ...
    def _apply_sweep_params(self, config, geometry, forcing, param_combo):
        """Apply a parameter combination dict to config/geometry/forcing objects in-place."""
        for key, value in param_combo.items():
            parts = key.split(".")
            if parts[0] == "config":
                setattr(config, parts[1], value)
            elif parts[0] == "geometry":
                setattr(geometry, parts[1], value)
            elif parts[0] == "forcing":
                setattr(forcing, parts[1], value)
            else:
                raise ValueError(f"Unknown object type in sweep parameter: {parts[0]}")

    def _generate_run_objects(self) -> List[Tuple[Any, Any, Any, dict]]:
        """Generate individual run objects from sweep parameters or spinup_objects.

        Returns a list of 4-tuples (config, geometry, forcing, param_combo) where
        param_combo is the dict of swept parameter values applied to that run.
        """
        ...
        if self.spinup_objects and not self.sweep_parameters:
            if isinstance(self.spinup_objects, dict):
                ...
                run_count = (
                    len(self.experimental_perturbations)
                    if self.experimental_perturbations
                    else 1
                )

            return [
                (self.base_config, self.base_geometry, self.base_forcing, {})
            ] * run_count
        ...
        for combination in itertools.product(*sweep_value_lists):
            ...
            param_combo = dict(zip(sweep_keys, combination))
            self._apply_sweep_params(config, geometry, forcing, param_combo)

            run_objects_list.append((config, geometry, forcing, param_combo))
    ...
    def _handle_spinup_objects(self, run_objects_list, client):
        ...
        if self.no_progress:
            ...
            with tqdm(
                total=len(task_list), desc="FlowlineSpinup runs", ncols=100
            ) as pbar:
                spinup_results = compute(task_list)[0]
            ...
            if isinstance(result, tuple):
                ...
                for param_name, param_value in optimized_parameters.items():
                    if hasattr(spinup_obj.forcing, param_name):
                        ...
                        logger.debug(
                            f"Applied optimized {param_name}={param_value:.3f} to spinup object"
                        )
            else:
        ...
        print(
            f"FlowlineSpinup objects completed. {len(run_spinup_mapping)} runs configured."
        )
        ...
        for param_path, perturbation_func in perturbations.items():
            ...
            if parts[0] == "config":
                ...
                if hasattr(perturbed_config, param_name):
                    ...
                    print(
                        f"Applied experimental perturbation {run_id}: {param_path} {current_value} -> {new_value}"
                    )
                ...
                if hasattr(perturbed_forcing, param_name):
                    ...
                    logger.debug(
                        f"{run_id}: Applied experimental perturbation {param_path} {current_value} -> {new_value}"
                    )
                    # Debug: Log all forcing parameters after perturbation
                    logger.debug(
                        f"{run_id}: Final forcing - T0={perturbed_forcing.T0:.3f}, gamma={perturbed_forcing.gamma:.6f}"
                    )
    ...
    def _get_spinup_forcing(self):
        ...
        if hasattr(spinup_forcing, "__dict__") and hasattr(
            spinup_forcing, "get_mass_balance"
        ):
    ...
    def _save_reproducibility_info(self):
        ...
        config_info = {
            ...
            else str(self.base_config),
            "base_geometry": {
                "x_gr_shape": self.base_geometry.x_gr.shape
                if hasattr(self.base_geometry, "x_gr")
                else None,
            ...
            "base_forcing": {
                ...
                "forcing_attrs": {
                    k: v
                    for k, v in self.base_forcing.__dict__.items()
    ...
    def _combine_results(self, successful_runs):
        ...
        def preprocess_ds(ds):
            ...
            if "run_id" in coords:
                ...
                ds = ds.assign(
                    {
                        "run_parameters": xr.DataArray(
                            ds.attrs["run_parameters"], dims=()
                        )
                    }
                )
        ...
        try:
            if sweep_dims:
                ...
                if self.sweep_parameters:
                    ...
                    sweep_value_lists = [
                        self.sweep_parameters[key] for key in sweep_dims
                    ]
                ...
                if "run_id" in sweep_dims:
                    ...
                    combined_ds = xr.open_mfdataset(
                        sorted_runs, preprocess=preprocess_ds, combine="by_coords"
                    )
                else:
    ...
    def run(self):
        ...
        # Get the keys from spinup_objects if they are the source of truth for run IDs
        spinup_keys = None
        if not self.sweep_parameters and isinstance(self.spinup_objects, dict):
            spinup_keys = list(self.spinup_objects.keys())

        for i, (config, geometry, forcing, param_combo) in enumerate(run_objects_list):
            # Determine the correct run_id for this iteration
            if spinup_keys:
                run_id = spinup_keys[i]
            else:
                run_id = self._get_run_id(i)
            ...
            task = dask.delayed(run_flowline_simulation)(
                (run_id, config, geometry, forcing, self.output_dir, self.no_progress)
            )
        ...
        if self.no_progress:
            ...
            with tqdm(
                total=len(exp_tasks), desc="Experimental runs", ncols=100
            ) as pbar:
                futures = compute(exp_tasks)[0]
                for future in futures:
                    results.append(future)
